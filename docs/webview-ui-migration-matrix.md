...
| `LatteOverlay.*`, `LatteShaderCache.*`, `swkbd.*`, `erreula.*` | runtime statistics, notifications, cache progress, software keyboard, error dialogs | `RuntimeOverlay::Model` | main-library / runtime-overlay | transparent in-process overlay | transparent WebView composition, render-surface pass-through, modal focus | C++ model tests + React tests + WebView/ImGui builds | In progress (Windows/macOS runtime validation) |
