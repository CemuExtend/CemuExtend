...
if(WIN32)
    ...
    function(z_vcpkg_wxwidgets_find_base_library BASENAME)
        if(MINGW AND BASENAME STREQUAL "scintilla")
            set(_debug_names wxscintillad-3.3 wxscintilla-3.3)
            set(_release_names wxscintilla-3.3)
        elseif(MINGW)
            set(_debug_names wx_${BASENAME}ud-3.3-Windows wx_${BASENAME}u-3.3-Windows)
            set(_release_names wx_${BASENAME}u-3.3-Windows)
        else()
            set(_debug_names wx${BASENAME}33ud wx${BASENAME}d)
            set(_release_names wx${BASENAME}33u wx${BASENAME})
        endif()
        find_library(WX_${BASENAME}d NAMES ${_debug_names} PATHS "${wxWidgets_ROOT_DIR}/debug/lib" NO_DEFAULT_PATH)
        find_library(WX_${BASENAME} NAMES ${_release_names} PATHS "${wxWidgets_ROOT_DIR}/lib" NO_DEFAULT_PATH REQUIRED)
    endfunction()
    function(z_vcpkg_wxwidgets_find_suffix_library BASENAME)
        foreach(lib IN LISTS ARGN)
            if(MINGW)
                find_library(WX_${lib}d NAMES wx_${BASENAME}ud_${lib}-3.3-Windows wx_${BASENAME}u_${lib}-3.3-Windows PATHS "${wxWidgets_ROOT_DIR}/debug/lib" NO_DEFAULT_PATH)
                find_library(WX_${lib} NAMES wx_${BASENAME}u_${lib}-3.3-Windows PATHS "${wxWidgets_ROOT_DIR}/lib" NO_DEFAULT_PATH)
            else()
