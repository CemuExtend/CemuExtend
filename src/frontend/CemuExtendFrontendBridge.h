...
namespace Frontend
{
	...
	class CemuExtendFrontendBridge final
	{
	  public:
		...
		[[nodiscard]] std::uint8_t PointerMode() const
		{
			return m_pointerMode;
		}
		[[nodiscard]] std::uint32_t MouseButtons() const
		{
			return m_mouseButtons;
		}
		[[nodiscard]] bool RawMouseRequested() const
		{
			return m_rawMouseRequested;
		}
		[[nodiscard]] bool RawMouseSeen() const
		{
			return m_rawMouseSeen;
		}

		[[nodiscard]] bool BeginTextInput(std::uint64_t sequence);
		void EndTextInput();
		void SetPreedit(std::string_view preedit);
		[[nodiscard]] bool CanSubmitText() const
		{
			return m_preedit.empty();
		}
		[[nodiscard]] bool HasPreedit() const
		{
			return !m_preedit.empty();
		}
		[[nodiscard]] std::uint64_t TextInputSequence() const
		{
			return m_textInputSequence;
		}
