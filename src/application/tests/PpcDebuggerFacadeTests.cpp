	...
	class FakeBackend final : public Application::IPpcDebuggerBackend
	{
	  public:
														...
														std::uint32_t count, std::uint32_t limit) override
		{
			...
			return {true, trapped, {0x1000}, {}, 0x2000, {{center.value ? center : Application::GuestAddress{0x1000}, 0x60000000, "nop", "", true, true}}, {{77, {0x1000}, true, false}}, false, ""};
		}
		...
		{
			assert(identity == 77 && address.value == 0x1000);
			enabled = value;
		}
		...
		void Control(Application::PpcDebuggerControl value) override
		{
			command = value;
		}
