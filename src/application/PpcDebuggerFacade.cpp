...
namespace Application
{
	namespace
	{
		...
		public:
			...
			~BreakpointLock() { debugger_unlockBreakpoints(); }
		...
		std::string FormatOperand(const PPCDisassemblerOperand& operand)
		{
			...
			switch (operand.type)
			{
			case PPCASM_OPERAND_TYPE_GPR: output << 'r' << unsigned(operand.registerIndex); break;
			case PPCASM_OPERAND_TYPE_FPR: output << 'f' << unsigned(operand.registerIndex); break;
			case PPCASM_OPERAND_TYPE_SPR: output << "spr" << unsigned(operand.registerIndex); break;
			case PPCASM_OPERAND_TYPE_CR: output << "cr" << unsigned(operand.registerIndex); break;
			case PPCASM_OPERAND_TYPE_CR_BIT: output << "crb" << unsigned(operand.registerIndex); break;
			case PPCASM_OPERAND_TYPE_CIMM:
				output << "0x" << std::hex << std::setw(8) << std::setfill('0') << operand.immU32; break;
			case PPCASM_OPERAND_TYPE_MEM:
				output << operand.immS32 << "(r" << unsigned(operand.registerIndex) << ')'; break;
			case PPCASM_OPERAND_TYPE_PSQMODE: output << (operand.immS32 ? "single" : "paired"); break;
			case PPCASM_OPERAND_TYPE_IMM:
			default:
				if (operand.isSignedImm) output << operand.immS32;
				else output << "0x" << std::hex << operand.immU32;
				break;
			}
			return output.str();
		}
		...
		public:
				...
				std::uint32_t instructionCount, std::uint32_t breakpointLimit) override
			{
				...
				if (center.value == 0) center = result.instructionPointer;
				...
				for (std::uint32_t index = 0; index < instructionCount; ++index)
				{
					...
					if (!memory_isAddressRangeAccessible(address, 4)) continue;
					std::uint32_t opcode = memory_readU32(address);
					bool hasBreakpoint = false;
					for (auto* bp = debugger_getFirstBP(address); bp; bp = bp->next)
						if (bp->isExecuteBP()) { opcode = bp->originalOpcodeValue; hasBreakpoint = true; break; }
					...
					{
						if ((decoded.operandMask & (1U << operandIndex)) == 0) continue;
						if (!first) operands << ", ";
						first = false;
					...
					result.instructions.push_back({{address}, opcode, std::move(mnemonic),
						operands.str(), address == result.instructionPointer.value, hasBreakpoint});
				}
				for (auto* head : lock.values)
					for (auto* bp = head; bp; bp = bp->next)
					{
						if (!bp->isExecuteBP() || bp->bpType == DEBUGGER_BP_T_ONE_SHOT) continue;
						if (result.breakpoints.size() == breakpointLimit)
						{
							result.breakpointCapReached = true;
							return result;
						}
						result.breakpoints.push_back({bp->id, {bp->address}, bp->enabled,
							bp->bpType == DEBUGGER_BP_T_LOGGING});
					}
				return result;
			}
			...
			{
				if (!CafeSystem::IsTitleRunning()) throw std::runtime_error("No Wii U title is running.");
				if (command == PpcDebuggerControl::Break)
				{
					if (!debugger_isTrapped()) debugger_requestBreak();
					return;
				}
				auto* cpu = debugger_lockDebugSession();
				if (!cpu) throw std::runtime_error("The PPC debugger is not paused.");
				const auto native = command == PpcDebuggerControl::Run ? DebuggerStepCommand::Run :
					command == PpcDebuggerControl::StepInto ? DebuggerStepCommand::StepInto : DebuggerStepCommand::StepOver;
	...
	{
		if (!m_backend) throw std::invalid_argument("PPC debugger backend is required");
	}

	void PpcDebuggerFacade::ValidateAddress(GuestAddress address)
	{
		if ((address.value & 3U) != 0) throw std::invalid_argument("guest address must be 4-byte aligned");
	}
	...
	{
		if (m_shuttingDown) throw std::runtime_error("PPC debugger is shutting down");
		...
		GuestAddress center, std::uint32_t instructionCount)
	{
		...
		result.available = backend.available; result.trapped = backend.trapped;
		result.instructionPointer = backend.instructionPointer; result.gpr = backend.gpr;
	...
	{
		ValidateAddress(address); std::scoped_lock lock(m_mutex);
		(void)RequireOwner(ownerWindow, generation); m_backend->ToggleExecuteBreakpoint(address);
	...
	{
		std::scoped_lock lock(m_mutex); auto& owner = RequireOwner(ownerWindow, generation);
		const auto found = owner.breakpoints.find(std::string(identity));
		if (found == owner.breakpoints.end()) throw std::invalid_argument("breakpoint identity is stale");
	...
	{
		std::scoped_lock lock(m_mutex); const auto& owner = RequireOwner(ownerWindow, generation);
		...
		m_backend->Control(command); m_owners.erase(ownerWindow);
	}

	void PpcDebuggerFacade::CloseOwner(std::uint64_t ownerWindow) noexcept
	{
		std::scoped_lock lock(m_mutex); m_owners.erase(ownerWindow);
	}

	void PpcDebuggerFacade::BeginShutdown() noexcept
	{
		std::scoped_lock lock(m_mutex); m_shuttingDown = true; m_owners.clear();
	}
