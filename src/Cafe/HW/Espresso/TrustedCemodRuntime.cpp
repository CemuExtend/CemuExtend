...
namespace
{
	...
	bool ContainsImageRange(const ParsedElf& elf, std::uint32_t virtualAddress, std::uint32_t size)
	{
		...
		for (const auto& segment : elf.segments)
			if (virtualAddress >= segment.virtualAddress &&
				...
				size <= segment.memorySize - (virtualAddress - segment.virtualAddress))
				return true;
		return false;
	}
						  ...
						  std::uint32_t loadBias, std::string& error)
	{
		...
		for (const auto& relocationSection : parsed.sections)
		{
			...
			for (std::uint32_t index = 0; index < relocationSection.size / relocationSection.entrySize; ++index)
			{
				...
				{
				case 0:
					break; // R_PPC_NONE: padding/no-op relocation
				case 1:
					if (absoluteSigned < 0 || absoluteSigned > std::numeric_limits<std::uint32_t>::max())
					{
						error = "R_PPC_ADDR32 overflow";
						return false;
					}
					Write32(target, static_cast<std::uint32_t>(absoluteSigned));
					break;
				case 4:
					Write16(target, static_cast<std::uint16_t>(absoluteSigned));
					break;
				case 5:
					Write16(target, static_cast<std::uint16_t>(static_cast<std::uint64_t>(absoluteSigned) >> 16));
					break;
				case 6:
					Write16(target, static_cast<std::uint16_t>((static_cast<std::uint64_t>(absoluteSigned) + 0x8000U) >> 16));
					break;
				case 10:
					if ((relativeSigned & 3) != 0 || relativeSigned < -0x02000000LL || relativeSigned > 0x01fffffcLL)
					{
						error = "R_PPC_REL24 overflow";
						return false;
					}
					Write32(target, (Read32(target) & 0xfc000003U) |
										(static_cast<std::uint32_t>(relativeSigned) & 0x03fffffcU));
					break;
				case 22:
				{
					...
					{
						error = "R_PPC_RELATIVE overflow";
						return false;
					}
					Write32(target, static_cast<std::uint32_t>(value));
					break;
				}
				case 26:
					...
					{
						error = "R_PPC_REL32 overflow";
						return false;
					}
					Write32(target, static_cast<std::uint32_t>(relativeSigned));
					break;
				default:
					error = fmt::format("unsupported trusted ELF relocation type {}", type);
					return false;
				}
			}
		}
	...
	class TrustedTitleOwner final : public cemuextend_hle::Cex2Owner
	{
	  public:
		...
		{
			return m_permissions.load(std::memory_order_acquire);
		}
		void SetGrantedPermissions(std::uint32_t value) override
		{
			m_permissions.store(value, std::memory_order_release);
		}
		void Stop()
		{
							  ...
							  std::uint16_t operation) const override
		{
			...
			if (permission == 1)
				return (m_read.load(std::memory_order_acquire) & bit) != 0;
			if (permission == 2)
				return (m_write.load(std::memory_order_acquire) & bit) != 0;
			if (permission == 4)
				return (m_inject.load(std::memory_order_acquire) & bit) != 0;
			if (permission == 8)
				return (((operation == 1 ? m_read : m_write).load(std::memory_order_acquire)) & bit) != 0;
			if (permission == 16)
				return (m_read.load(std::memory_order_acquire) & bit) != 0;
			return false;
		}
													   ...
													   std::uint32_t titlePermissions, const ModServicePermissions& services, std::string& error)
{
	...
	{
		error = "package is not trusted_native";
		return std::nullopt;
	}
	...
	{
		error = "trusted runtime already belongs to another title";
		return std::nullopt;
	}
	for (const auto& [handle, mod] : m_impl->mods)
		if (mod.package.manifest.modId == package.manifest.modId)
		{
			error = "duplicate trusted native mod_id";
			return std::nullopt;
		}

	ParsedElf parsed;
	if (!ParseElf(package, parsed, error))
		return std::nullopt;
	const auto allocation = RPLLoader_AllocateCodeCaveMem(256, parsed.imageSize);
	if (!allocation)
	{
		error = "shared codecave does not have enough space";
		return std::nullopt;
	}
	...
	{
		error = "trusted ELF load bias underflow";
		release();
		return std::nullopt;
	}
	...
	{
		error = "invalid CMB1 bootstrap header";
		release();
		return std::nullopt;
	}
	...
	{
		error = "invalid CMB1 bootstrap record count";
		release();
		return std::nullopt;
	}
	...
	for (std::uint32_t record = 0; record < recordCount; ++record)
	{
		...
		{
			error = "invalid CMB1 bootstrap record";
			release();
			return std::nullopt;
		}
		for (sint32 moduleIndex = 0; moduleIndex < RPLLoader_GetModuleCount(); ++moduleIndex)
		{
			...
			if (!module || module->patchCRC != moduleHash)
				continue;
			const auto target = ResolveModuleAddress(*module, targetVirtual);
			if (!target)
			{
				error = "bootstrap target is outside the matching module";
				release();
				return std::nullopt;
			}
			if (match)
			{
				error = "bootstrap records match more than one loaded module";
				release();
				return std::nullopt;
			}
	...
	if (!match)
	{
		error = "no CMB1 bootstrap record matches a loaded module";
		release();
		return std::nullopt;
	}
	...
	{
		error = "bootstrap patch target is invalid or already claimed";
		release();
		return std::nullopt;
	}
	...
	{
		error = "bootstrap target instruction does not match the package";
		release();
		return std::nullopt;
	}
	...
	{
		error = "bootstrap branch is outside the PPC REL24 range";
		release();
		return std::nullopt;
	}
