...
class ModExecutionContext final : public cemuextend_hle::Cex2Owner
{
  public:
	...
	[[nodiscard]] std::uint64_t AddressSpaceId() const override
	{
	...
	[[nodiscard]] const std::string& Principal() const override
	{
		return m_principal;
	}
	[[nodiscard]] std::uint32_t VirtualBase() const
	{
		return m_virtualBase;
	}
	[[nodiscard]] std::uint32_t AddressSpaceSize() const
	{
		return static_cast<std::uint32_t>(m_memory.size());
	}
	...
	void SetTitleId(std::uint64_t titleId)
	{
		m_titleId = titleId;
	}
	[[nodiscard]] std::uint64_t TitleId() const override
	{
		return m_titleId;
	}
