...
namespace cemuextend_hle
{
	...
	constexpr std::uint64_t kCemodServiceMicrophoneApproval = 1ULL << 17U;
	...
	constexpr std::uint64_t kCemodExactApprovalPermissionMask =
		(kCemodServiceMicrophoneApproval << 1U) - 1U;
		...
		std::uint64_t exactGranted, std::uint32_t serviceRequested, bool nativePackage)
	{
		...
		if (nativePackage)
		{
			...
			constexpr std::pair<std::uint64_t, std::uint32_t> services[]{
				...
				{kCemodServiceMicrophoneApproval, kCemodMicrophonePermission},
			};
		...
		else
		{
			...
			if ((serviceRequested & kCemodMicrophonePermission) != 0 &&
				(exactGranted & kCemodServiceMicrophoneApproval) != 0)
				result |= kCemodMicrophonePermission;
		}
