...
namespace cemuextend_hle
{
	namespace
	{
		...
		Fd OpenParent(int root, const std::vector<std::string>& parts, bool create)
		{
			...
			for (std::size_t index = 0; index + 1 < parts.size(); ++index)
			{
				...
				{
					if (::mkdirat(current.get(), parts[index].c_str(), 0700) != 0 && errno != EEXIST)
						return {};
		...
		bool Measure(int directory, Usage& usage)
		{
			...
			while (auto* entry = ::readdir(stream))
			{
				...
				else if (S_ISDIR(stat.st_mode))
				{
					...
					if (!child || !Measure(child.get(), usage))
					{
						::closedir(stream);
						return false;
					}
		...
		Status ErrnoStatus(int value)
		{
			...
			if (value == EEXIST || value == ENOTEMPTY)
				return Status::Busy;
			if (value == ELOOP || value == ENOTDIR || value == EPERM || value == EACCES)
		...
		Status WinStatus(DWORD value)
		{
			...
			if (value == ERROR_ACCESS_DENIED || value == ERROR_CANT_ACCESS_FILE || value == ERROR_REPARSE_TAG_INVALID ||
				value == ERROR_NOT_A_REPARSE_POINT)
				return Status::PermissionDenied;
			return Status::IoError;
		}
		...
		WinHandle OpenRoot(const fs::path& root)
		{
			...
			if (error)
				return {};
			WinHandle result(::CreateFileW(root.c_str(), FILE_LIST_DIRECTORY | FILE_READ_ATTRIBUTES | FILE_TRAVERSE | FILE_ADD_FILE | FILE_ADD_SUBDIRECTORY | FILE_DELETE_CHILD | SYNCHRONIZE,
			...
			for (std::size_t index = 0; index + 1 < parts.size(); ++index)
			{
				auto next = OpenRelative(current.get(), parts[index], true, create,
										 FILE_LIST_DIRECTORY | FILE_READ_ATTRIBUTES | FILE_TRAVERSE |
											 (create ? FILE_ADD_FILE | FILE_ADD_SUBDIRECTORY | FILE_DELETE_CHILD : 0),
										 error);
				if (!next)
					return {};
		...
		bool Measure(HANDLE directory, Usage& usage)
		{
			...
			for (const auto& entry : entries)
			{
				...
				{
					if (entry.size > kNamespaceBytes || usage.bytes > kNamespaceBytes - entry.size)
						return false;
					usage.bytes += entry.size;
					if (++usage.files > kMaximumFiles)
						return false;
				}
			}
			return true;
		...
		bool ValidConfigValue(ValueType type, std::span<const std::byte> value)
		{
			...
			case ValueType::Boolean:
				return value.size() == 1 && std::to_integer<unsigned>(value[0]) <= 1;
		...
		Cex2StorageResult LoadConfig(const fs::path& root, ConfigMap& values)
		{
			...
			if (!input)
				return errno == ENOENT ? Cex2StorageResult{Status::Ok} : Cex2StorageResult{Status::IoError};
			struct stat stat{};
			if (::fstat(input.get(), &stat) != 0 || !S_ISREG(stat.st_mode) || stat.st_size < 0 ||
				static_cast<std::uint64_t>(stat.st_size) > kMaximumConfigBytes + 48)
				return {Status::IoError};
			...
			if (!input)
				return error == ERROR_FILE_NOT_FOUND ? Cex2StorageResult{Status::Ok} : Cex2StorageResult{Status::IoError};
			FILE_STANDARD_INFO standard{};
			...
			if (!bytes.empty() && (!::ReadFile(input.get(), bytes.data(), static_cast<DWORD>(bytes.size()), &read, nullptr) ||
				...
				return {Status::IoError};
#endif
			if (bytes.size() < 48 || std::memcmp(bytes.data(), "CEX2CFG\0", 8) != 0)
				return {Status::IoError};
		...
		Cex2StorageResult SaveConfig(const fs::path& root, const ConfigMap& values)
		{
			...
			{
				payload.String(key);
				payload.U8(static_cast<std::uint8_t>(typed.first));
				payload.U32(static_cast<std::uint32_t>(typed.second.size()));
				payload.Bytes(typed.second);
			}
			Encoder file;
			file.Bytes({reinterpret_cast<const std::byte*>("CEX2CFG\0"), 8});
			file.U16(1);
			file.U16(0x4245);
			file.U32(static_cast<std::uint32_t>(payload.data().size()));
			const auto digest = Digest(payload.data());
			file.Bytes({reinterpret_cast<const std::byte*>(digest.data()), digest.size()});
			...
			if (!::WriteFile(output.get(), file.data().data(), static_cast<DWORD>(file.data().size()),
							 &written, nullptr) ||
				written != file.data().size() || !::FlushFileBuffers(output.get()))
				return {Status::IoError};
								 ...
								 std::span<const std::byte> payload)
		{
			...
			if (!decoder.String(key) || key.size() > 256 || !IsValidUtf8(key) ||
				(key.empty() && operation != static_cast<std::uint16_t>(ConfigurationOperation::List)))
			...
			{
				ConfigMap values;
				if (auto loaded = LoadConfig(root, values); loaded.status != Status::Ok)
					return loaded;
			...
			if (operation == static_cast<std::uint16_t>(ConfigurationOperation::Get))
			{
				...
				if (found == values.end())
					return {Status::NotFound};
				Encoder out;
				out.U8(static_cast<std::uint8_t>(found->second.first));
				out.U32(found->second.second.size());
				out.Bytes(found->second.second);
			...
			if (operation == static_cast<std::uint16_t>(ConfigurationOperation::Set))
			{
				...
				for (const auto& [other, item] : values)
					if (other != key)
						total += other.size() + item.second.size();
				if (total > kMaximumConfigBytes)
					return {Status::TooLarge};
			...
			if (operation == static_cast<std::uint16_t>(ConfigurationOperation::Delete))
			{
				...
				auto saved = SaveConfig(root, values);
				if (saved.status != Status::Ok)
					values.insert(std::move(removed));
				return saved;
			}
			if (operation != static_cast<std::uint16_t>(ConfigurationOperation::List))
			...
			if (!decoder.U16(maximum) || !decoder.U32(tokenSize) || !decoder.Bytes(tokenSize, token) || decoder.remaining() ||
				maximum == 0 || maximum > kMaximumPageEntries || tokenSize > 256)
				return {Status::InvalidArgument};
			const std::string after(reinterpret_cast<const char*>(token.data()), token.size());
			Encoder out;
			std::vector<ConfigMap::const_iterator> page;
			...
			if (!page.empty())
			{
				auto next = std::next(page.back());
				truncated = next != values.end() && next->first.starts_with(key);
			}
			out.U32(page.size());
			for (auto it : page)
			{
				out.String(it->first);
				out.U8(static_cast<std::uint8_t>(it->second.first));
			}
			out.U8(truncated);
			const auto nextToken = truncated ? page.back()->first : std::string{};
			out.U32(nextToken.size());
			out.Bytes({reinterpret_cast<const std::byte*>(nextToken.data()), nextToken.size()});
			return {Status::Ok, out.Take()};
		}
							   ...
							   std::span<const std::byte> payload)
		{
			...
			if (!parts)
				return {Status::PermissionDenied};
			Fd root = OpenRoot(NamespaceRoot(title, principal, "files"));
			if (!root)
				return {Status::IoError};
			if (operation == static_cast<std::uint16_t>(FileOperation::Mkdir))
			{
				if (decoder.remaining() || parts->empty())
					return {Status::InvalidArgument};
				Fd current(::dup(root.get()));
				for (const auto& part : *parts)
				{
					if (::mkdirat(current.get(), part.c_str(), 0700) != 0 && errno != EEXIST)
			...
			if (parts->empty() && !list)
				return {Status::PermissionDenied};
			Fd parent = parts->empty() ? Fd(::dup(root.get())) : OpenParent(root.get(), *parts, operation == static_cast<std::uint16_t>(FileOperation::Write));
			if (!parent)
				return {ErrnoStatus(errno)};
			...
			if (operation == static_cast<std::uint16_t>(FileOperation::Stat))
			{
				if (decoder.remaining())
					return {Status::InvalidArgument};
				struct stat stat{};
				if (::fstatat(parent.get(), name, &stat, AT_SYMLINK_NOFOLLOW) != 0)
					return {ErrnoStatus(errno)};
			...
			if (list)
			{
				...
				std::uint32_t tokenSize{};
				std::span<const std::byte> token;
				if (!decoder.U16(maximum) || !decoder.U32(tokenSize) || !decoder.Bytes(tokenSize, token) || decoder.remaining() || maximum == 0 || maximum > kMaximumPageEntries || tokenSize > 255)
					return {Status::InvalidArgument};
				Fd directory(parts->empty() ? ::dup(root.get()) : ::openat(parent.get(), name, O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW));
				if (!directory)
					return {ErrnoStatus(errno)};
				...
				while (auto* item = ::readdir(stream))
				{
					if (!std::strcmp(item->d_name, ".") || !std::strcmp(item->d_name, ".."))
						continue;
					struct stat stat{};
					if (::fstatat(directory.get(), item->d_name, &stat, AT_SYMLINK_NOFOLLOW) != 0 || (!S_ISREG(stat.st_mode) && !S_ISDIR(stat.st_mode)))
					{
						::closedir(stream);
						return {Status::IoError};
					}
					entries.push_back({item->d_name, S_ISDIR(stat.st_mode) ? std::uint8_t{2} : std::uint8_t{1}, S_ISREG(stat.st_mode) ? static_cast<std::uint64_t>(stat.st_size) : 0});
				}
				if (::closedir(stream) != 0)
					return {Status::IoError};
				std::ranges::sort(entries, {}, &Entry::name);
				const std::string after(reinterpret_cast<const char*>(token.data()), token.size());
				auto begin = std::ranges::upper_bound(entries, after, {}, &Entry::name);
			...
			if (operation == static_cast<std::uint16_t>(FileOperation::Read))
			{
				std::uint64_t offset{};
				std::uint32_t size{};
				if (!decoder.U64(offset) || !decoder.U32(size) || decoder.remaining() || size > kMaximumValueBytes || offset > kMaximumFileBytes)
					return {Status::InvalidArgument};
				Fd file(::openat(parent.get(), name, O_RDONLY | O_CLOEXEC | O_NOFOLLOW));
				if (!file)
					return {ErrnoStatus(errno)};
				struct stat stat{};
				if (::fstat(file.get(), &stat) != 0 || !S_ISREG(stat.st_mode))
					return {Status::PermissionDenied};
				if (offset > static_cast<std::uint64_t>(stat.st_size))
					return {Status::InvalidArgument};
				std::vector<std::byte> out(std::min<std::uint64_t>(size, stat.st_size - offset));
				const auto count = ::pread(file.get(), out.data(), out.size(), static_cast<off_t>(offset));
				if (count < 0)
					return {Status::IoError};
				out.resize(count);
			...
			{
				std::uint64_t offset{};
				if (!decoder.U64(offset) || decoder.remaining() > kMaximumValueBytes || offset > kMaximumFileBytes || decoder.remaining() > kMaximumFileBytes - offset)
					return {Status::InvalidArgument};
				std::span<const std::byte> data;
				decoder.Bytes(decoder.remaining(), data);
				Usage usage{};
				if (!Measure(root.get(), usage))
					return {Status::IoError};
				struct stat before{};
				const bool exists = ::fstatat(parent.get(), name, &before, AT_SYMLINK_NOFOLLOW) == 0;
				if (exists && (!S_ISREG(before.st_mode) || before.st_size < 0))
					return {Status::PermissionDenied};
				const std::uint64_t oldSize = exists ? static_cast<std::uint64_t>(before.st_size) : 0;
				const auto newSize = std::max<std::uint64_t>(oldSize, offset + data.size());
				if (offset > oldSize)
					return {Status::InvalidArgument};
				if (oldSize > usage.bytes)
					return {Status::IoError};
				if ((!exists && usage.files >= kMaximumFiles) || newSize > kMaximumFileBytes ||
					usage.bytes - oldSize > kNamespaceBytes - newSize)
					return {Status::TooLarge};
				Fd file(::openat(parent.get(), name, O_WRONLY | O_CREAT | O_CLOEXEC | O_NOFOLLOW, 0600));
				if (!file)
					return {ErrnoStatus(errno)};
				std::size_t done{};
				while (done < data.size())
				{
					const auto count = ::pwrite(file.get(), data.data() + done, data.size() - done, static_cast<off_t>(offset + done));
					if (count <= 0)
						return {Status::IoError};
					done += count;
				}
				return ::fsync(file.get()) == 0 ? Cex2StorageResult{Status::Ok} : Cex2StorageResult{Status::IoError};
			}
			if (operation == static_cast<std::uint16_t>(FileOperation::Remove))
			{
				...
				if (!S_ISREG(stat.st_mode) && !S_ISDIR(stat.st_mode))
					return {Status::PermissionDenied};
				if (::unlinkat(parent.get(), name, S_ISDIR(stat.st_mode) ? AT_REMOVEDIR : 0) != 0)
					return {ErrnoStatus(errno)};
				return {Status::Ok};
			}
							   ...
							   std::span<const std::byte> payload)
		{
			...
			if (parts->empty() && !list)
				return {Status::PermissionDenied};
			auto parent = parts->empty() ? Duplicate(root.get()) : OpenParent(root.get(), *parts, operation == static_cast<std::uint16_t>(FileOperation::Write), error);
			if (!parent)
				return {WinStatus(error)};
			...
			if (operation == static_cast<std::uint16_t>(FileOperation::Stat))
			{
				...
				const auto ticks = basic.LastWriteTime.QuadPart > 0 ? static_cast<std::uint64_t>(basic.LastWriteTime.QuadPart) : 0;
			...
			if (list)
			{
				...
				if (!decoder.U16(maximum) || !decoder.U32(tokenSize) || !decoder.Bytes(tokenSize, token) ||
					...
					return {Status::InvalidArgument};
				auto directory = parts->empty() ? Duplicate(root.get()) : OpenRelative(parent.get(), name, true, false, FILE_LIST_DIRECTORY | FILE_READ_ATTRIBUTES | FILE_TRAVERSE, error);
				if (!directory)
					return {WinStatus(error)};
				std::vector<WinEntry> entries;
				if (!Entries(directory.get(), entries))
					return {Status::IoError};
			...
			if (operation == static_cast<std::uint16_t>(FileOperation::Read))
			{
				...
				if (!::GetFileInformationByHandleEx(file.get(), FileStandardInfo, &standard, sizeof(standard)) ||
					offset > static_cast<std::uint64_t>(standard.EndOfFile.QuadPart))
					return {Status::InvalidArgument};
			...
			if (operation == static_cast<std::uint16_t>(FileOperation::Write))
			{
				...
				if (!existed && error != ERROR_FILE_NOT_FOUND && error != ERROR_PATH_NOT_FOUND)
					return {WinStatus(error)};
				if (!existed && usage.files >= kMaximumFiles)
					return {Status::TooLarge};
				...
				Usage afterOpen{};
				if (!Measure(root.get(), afterOpen) || afterOpen.files > kMaximumFiles)
					return {Status::TooLarge};
				LARGE_INTEGER position{};
				position.QuadPart = static_cast<LONGLONG>(offset);
				if (!::SetFilePointerEx(file.get(), position, nullptr, FILE_BEGIN))
				...
				return ::FlushFileBuffers(file.get()) ? Cex2StorageResult{Status::Ok} : Cex2StorageResult{Status::IoError};
			}
			if (operation == static_cast<std::uint16_t>(FileOperation::Remove))
			{
				if (decoder.remaining())
											...
											ServiceId service, std::uint16_t operation, std::span<const std::byte> payload)
	{
		...
		if (service == ServiceId::Configuration)
			return Config(titleId, principal, operation, payload);
		if (service == ServiceId::File)
			return File(titleId, principal, operation, payload);
		return {Status::NotSupported};
	}
	...
	Status Cex2Storage::ImportLegacy(std::uint64_t titleId, std::string_view principal)
	{
		...
		if (!hasFiles && !hasConfig)
			return Status::NotFound;
		if (fs::exists(newFiles, error) ||
			(fs::exists(newConfig / "config.cxc2", error)))
			return Status::Busy;
		...
		if (hasConfig)
		{
			...
			for (std::uint32_t index = 0; index < count; ++index)
			{
				...
				if (!input || key.empty() || !IsValidUtf8(key) || values.contains(key) ||
					!ValidConfigValue(valueType, value))
					return Status::IoError;
				total += key.size() + value.size();
		...
		if (hasConfig)
		{
			...
			if (saved.status != Status::Ok)
			{
				std::error_code cleanup;
				if (hasFiles)
					fs::remove_all(newFiles, cleanup);
				cleanup.clear();
