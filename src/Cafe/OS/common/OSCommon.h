...
#define ppc_define_va_list(__gprIndex, __fprIndex)                                               \
	...
	for (int i = 3; i <= 10; i++)                                                                \
		va_list_storage->gpr_save_area[i - 3] = PPCInterpreter_getCurrentInstance()->gpr[i];     \
	for (int i = 1; i <= 8; i++)                                                                 \
		va_list_storage->fpr_save_area[i - 1] = PPCInterpreter_getCurrentInstance()->fpr[i].fp0; \
