...
void GamePatch_scan()
{
	...
	if (service != NetworkService::Nintendo // Only patch for custom services.
		&& CafeSystem::GetForegroundTitleArgStr().ends_with("wave.rpx") && (hleAddr = hle_locate(miiverse_eshop_url_match_whitelist_func, nullptr, sizeof(miiverse_eshop_url_match_whitelist_func))))
	{
	...
	if (IsNetworkServiceSSLDisabled(service) && RPLLoader_GetHandleByModuleName("libopenssl.rpl") != RPL_INVALID_HANDLE && (hleAddr = hle_locate(wave_libopenssl_ssl_verify_cert_chain, nullptr, sizeof(wave_libopenssl_ssl_verify_cert_chain))))
	{
