...
class Account
{
  public:
	...
	{
		return fmt::format(L"{} ({:x})", GetMiiName(), GetPersistentId());
	}
	...
	{
		return m_persistent_id;
	}
	[[nodiscard]] uint64 GetTransferableIdBase() const
	{
		return m_transferable_id_base;
	}
	[[nodiscard]] const std::array<uint8, 16>& GetUuid() const
	{
		return m_uuid;
	}
	[[nodiscard]] const std::array<uint8, 96>& GetMiiData() const
	{
		return m_mii_data;
	}
	[[nodiscard]] std::wstring_view GetMiiName() const; // only max 10 characters excluding '\0'
	[[nodiscard]] std::string_view GetAccountId() const
	{
		return m_account_id;
	}
	[[nodiscard]] uint16 GetBirthYear() const
	{
		return m_birth_year;
	}
	[[nodiscard]] uint8 GetBirthMonth() const
	{
		return m_birth_month;
	}
	[[nodiscard]] uint8 GetBirthDay() const
	{
		return m_birth_day;
	}
	...
	[[nodiscard]] uint32 GetCountry() const
	{
		return m_country;
	}
	[[nodiscard]] uint32 GetSimpleAddressId() const
	{
		return m_simple_address_id;
	}
	[[nodiscard]] std::string_view GetTimeZoneId() const
	{
		return m_timezone_id;
	}
	[[nodiscard]] sint64 GetUtcOffset() const
	{
		return m_utc_offset;
	}
	[[nodiscard]] uint32 GetPrincipalId() const
	{
		return m_principal_id;
	}
	[[nodiscard]] bool IsPasswordCacheEnabled() const
	{
		return m_password_cache_enabled != 0;
	}
	[[nodiscard]] const std::array<uint8, 32>& GetAccountPasswordCache() const
	{
		return m_account_password_cache;
	}

	[[nodiscard]] std::string_view GetStorageValue(std::string_view key) const;

	void SetMiiName(std::wstring_view name);
	void SetBirthYear(uint16 birth_year)
	{
		m_birth_year = birth_year;
	}
	void SetBirthMonth(uint8 birth_month)
	{
		m_birth_month = birth_month;
	}
	void SetBirthDay(uint8 birth_day)
	{
	...
	void SetCountry(uint32 country)
	{
		m_country = country;
	}
	void SetTimeZoneId(std::string_view timezone_id)
	{
		m_timezone_id = timezone_id;
	}
	void SetUtcOffset(sint64 utc_offset)
	{
