	...
	bool headless)
{
	const auto inspection = CemuExtend::CemodInspectionService::Inspect({
		info.path, info.modId, info.principal, info.requestedPermissions, info.executionMode,
		info.signedPackage, info.titleIds, info.error}, approval, headless);
	...
	view.payloadFormat = inspection.isWups ? "WUPS plugin.wps" :
		(info.executionMode == CemodGuiExecutionMode::TrustedNative ?
			"Cemu ELF mod.elf" : "cemod_elf");
	...
	case CemuExtend::CemodScope::Title:
		view.scope = "title " + (titleId == 0 ? std::string("all configured titles") :
			[&] { std::ostringstream stream; stream << std::hex << titleId; return stream.str(); }());
		break;
	...
	view.abiWarning = view.wupsAbiVersion == "0.9.1" ? std::string{} :
		"Legacy WUPS ABI; runtime compatibility handling is not connected";
