...
namespace WebFrontend
{
	namespace
	{
			...
			std::string_view title, std::string_view suggested)
		{
			...
			request.hwndOwner = static_cast<HWND>(owner); request.lpstrFile = buffer.data();
			...
			if (!(save ? GetSaveFileNameW(&request) : GetOpenFileNameW(&request))) return {};
	...
	std::optional<std::filesystem::path> SelectArchiveToSave(void* owner, std::string_view title,
		std::string_view suggested) { return Select(owner, true, title, suggested); }
}
#endif
