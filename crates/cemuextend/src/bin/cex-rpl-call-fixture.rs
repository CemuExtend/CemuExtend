//! Emits one exact synthetic RPX/RPL imported-call fixture.
...
const HELP: &str = "Usage: cex-rpl-call-fixture --module main|provider --output FILE|-\n";
...
fn fixture(module: Module) -> Result<Vec<u8>, FixtureError> {
    match module {
        Module::Main => cex_system::synthetic_rpx_rpl_call_main_fixture(),
        Module::Provider => cex_system::synthetic_rpx_rpl_call_provider_fixture(),
    }
...
fn publish_fixture(destination: &OsStr, bytes: &[u8]) -> Result<(), FixtureError> {
    ...
    let mut staged = tempfile::NamedTempFile::new_in(parent).map_err(|_| FixtureError::Output)?;
    staged
        .as_file_mut()
        .write_all(bytes)
...
impl std::fmt::Display for FixtureError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            Self::Usage => "cex-rpl-call-fixture: invalid arguments",
            Self::Output => "cex-rpl-call-fixture: failed to write fixture",
            Self::Harness => "cex-rpl-call-fixture: failed to construct fixture",
        })
    }
}
...
mod tests {
    ...
    fn selectors_emit_the_exact_production_fixtures() {
        assert_eq!(
            ...
            cex_system::synthetic_rpx_rpl_call_main_fixture().unwrap()
        );
        assert_eq!(
            fixture(Module::Provider).unwrap(),
            cex_system::synthetic_rpx_rpl_call_provider_fixture().unwrap()
        );
    }
