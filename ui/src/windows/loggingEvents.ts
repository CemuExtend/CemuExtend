...
export function loggingSnapshotFromEvent(event: NativeEvent): LoggingSnapshot | undefined {
  ...
  if (!Array.isArray(value.entries) || typeof value.nextSequence !== "string" || !decimalId.test(value.nextSequence) ||
    typeof value.firstAvailableSequence !== "string" || !decimalId.test(value.firstAvailableSequence) ||
    typeof value.droppedEntries !== "string" || !decimalId.test(value.droppedEntries) ||
    typeof value.retainedBytes !== "string" || !decimalId.test(value.retainedBytes) ||
    typeof value.truncated !== "boolean") return undefined;
  if (!value.entries.every((entry) => entry && typeof entry.sequence === "string" && decimalId.test(entry.sequence) &&
...
export function mergeLoggingEntries(current: LoggingEntry[], incoming: LoggingEntry[]): LoggingEntry[] {
  ...
  return [...bySequence.values()].sort((left, right) => {
    const lhs = BigInt(left.sequence); const rhs = BigInt(right.sequence);
