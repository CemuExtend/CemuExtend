...
for (let index = 0; index < 26; index += 1) codeUsage.set(`Key${String.fromCharCode(65 + index)}`, 0x04 + index);
for (let index = 1; index <= 9; index += 1) codeUsage.set(`Digit${index}`, 0x1d + index);
...
[
  ...
  ["Numpad9", 0x61], ["Numpad0", 0x62], ["NumpadDecimal", 0x63], ["ContextMenu", 0x65]
].forEach(([code, usage]) => codeUsage.set(code as string, usage as number));
for (let index = 1; index <= 12; index += 1) codeUsage.set(`F${index}`, 0x39 + index);
for (let index = 13; index <= 24; index += 1) codeUsage.set(`F${index}`, 0x68 + index - 13);
const usageCode = new Map(Array.from(codeUsage, ([code, usage]) => [usage, code]));

export const hotkeyUsageForCode = (code: string) => codeUsage.get(code);

export function hotkeyKeyboardLabel(binding: { keyboardUsage: number; keyboardModifiers: number }) {
  ...
  parts.push(usageCode.get(binding.keyboardUsage)?.replace(/^Key/, "").replace(/^Digit/, "") ?? `HID 0x${binding.keyboardUsage.toString(16)}`);
  return parts.join(" + ");
}
