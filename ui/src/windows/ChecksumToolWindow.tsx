...
export function ChecksumToolWindow({ windowId }: { windowId: number }) {
  ...
  const start = async () => { if (!selectedUid) return; setError(""); setResult(undefined); setProgress({ phase: "starting", filesCompleted: 0, filesTotal: 0, bytesCompleted: 0, bytesTotal: 0 }); try { const started = await invoke("checksum.start", { locationUid: selectedUid }); jobId.current = started.jobId; activateJob(pendingEvents.current, started.jobId, (event) => applyJobEvent(event.type, event.payload)); } catch (reason) { setProgress(undefined); setError(String(reason)); } };
  ...
  return <main className="role-window manager-window"><header><div><span className="eyebrow">Content integrity</span><h1>Checksum Tool</h1></div><div className="button-row"><button disabled={!selected || !!progress} onClick={() => void start()}>Generate checksum</button>{progress && <button onClick={cancel}>Cancel</button>}<button onClick={() => void invoke("window.close")}>Close</button></div></header>
    {error && <div className="notice error" role="alert">{error}</div>}<div className="toolbar embedded"><input type="search" placeholder="Filter installed content" value={query} onChange={(event) => setQuery(event.target.value)} /><span>{entries.length} installations</span></div>
    <div className="split-view"><aside className="selection-list">{entries.map((entry: ChecksumContent) => <button key={entry.locationUid} className={entry.locationUid === selectedUid ? "selected" : ""} disabled={!!progress} onClick={() => { setSelectedUid(entry.locationUid); setResult(undefined); }}><strong>{entry.name || entry.titleId}</strong><span>{entry.type} · {entry.format} · v{entry.version}</span><code>{entry.titleId}</code></button>)}</aside><section className="editor-panel">{selected ? <><h2>{selected.name || selected.titleId}</h2><dl className="detail-list compact"><div><dt>Title ID</dt><dd><code>{selected.titleId}</code></dd></div><div><dt>Region</dt><dd>{selected.region}</dd></div><div><dt>Content</dt><dd>{selected.type} / {selected.format}</dd></div></dl>{progress && <><progress max={100} value={percent} /><p>{progress.phase}: {progress.filesCompleted}/{progress.filesTotal} files · {Math.round(progress.bytesCompleted / 1048576)}/{Math.round(progress.bytesTotal / 1048576)} MiB</p></>}{result && <><div className="notice">Checksum generated for {result.files.length ? `${result.files.length} files` : "the game image"}.</div>{result.imageSha256 && <p><strong>Image SHA-256</strong><br/><code>{result.imageSha256}</code></p>}<div className="checksum-results">{result.files.map((file) => <div key={file.path}><code>{file.sha256}</code><span>{file.path}</span></div>)}</div></>}</> : <p>No installed content was found.</p>}</section></div></main>;
}
