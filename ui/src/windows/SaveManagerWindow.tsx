...
export function SaveManagerWindow({ windowId }: { windowId: string }) {
  const [model, setModel] = useState<SaveManagerModel>({
    scanning: false,
    accounts: [],
    titles: [],
  });
  ...
  const load = useCallback(async () => {
    const next = await invoke("save.getModel");
    setModel(next);
    setTitleId((current) =>
      next.titles.some((title) => title.titleId === current)
        ? current
        : (next.titles[0]?.titleId ?? ""),
    );
  }, []);
  const selectedTitle = model.titles.find((title) => title.titleId === titleId);
  const selectedSave = selectedTitle?.saves.find(
    (save) => save.persistentId === persistentId,
  );
  useEffect(() => {
    if (
      !selectedTitle?.saves.some((save) => save.persistentId === persistentId)
    )
      setPersistentId(selectedTitle?.saves[0]?.persistentId ?? "");
  ...
  const applyJobEvent = useCallback(
    (event: JobEvent) => {
      ...
      if (payload.ok === true) {
        setStatus(`${String(payload.operation)} completed`);
        void load();
      } else
        setError(String(payload.diagnostic || "Save archive operation failed"));
    },
    [load],
  );
  useEffect(() => {
  ...
  const activate = (started: { jobId: string }) => {
  ...
  const remove = () =>
    run(async () => {
      ...
      const prepared = await invoke("save.delete.prepare", {
        titleId: selectedTitle.titleId,
        persistentId: selectedSave.persistentId,
      });
      if (
        !window.confirm(
          `Permanently delete save ${selectedSave.persistentId} for ${selectedTitle.name || selectedTitle.titleId}?`,
        )
      )
        return;
      await invoke("save.delete", prepared);
  ...
  const transfer = () =>
    run(async () => {
      if (!selectedSave || !selectedTitle) return;
      const inspected = await invoke("save.transfer.inspect", {
        titleId: selectedTitle.titleId,
        sourcePersistentId: selectedSave.persistentId,
        targetPersistentId: targetId,
      });
      const overwrite = inspected.targetState === "directory";
      if (
        !window.confirm(
          `${overwrite ? "Overwrite the existing target save and move" : "Move"} ${selectedSave.persistentId} to ${targetId}?`,
        )
      )
        return;
      await invoke("save.transfer", {
        confirmationToken: inspected.confirmationToken,
      });
      setStatus("Save transferred");
      await load();
    });
  const importArchive = () =>
    run(async () => {
      if (!selectedTitle) return;
      const picked = await invoke("save.import.pick", {
        titleId: selectedTitle.titleId,
      });
      if (!picked.selected || !picked.fileToken) return;
      const target = targetId || model.accounts[0]?.persistentId || "";
      const inspected = await invoke("save.import.inspect", {
        fileToken: picked.fileToken,
        titleId: selectedTitle.titleId,
        persistentId: target,
      });
      const warnings = [
        inspected.titleMismatch
          ? `Archive title ${inspected.sourceTitleId ?? "unknown"} differs.`
          : "",
        inspected.targetState === "directory"
          ? "The target save will be overwritten."
          : "",
      ]
        .filter(Boolean)
        .join("\n");
      if (
        !window.confirm(
          `Import ${picked.name ?? "archive"} to account ${target}?${warnings ? `\n\n${warnings}` : ""}`,
        )
      )
        return;
      activate(
        await invoke("save.import.start", {
          confirmationToken: inspected.confirmationToken,
        }),
      );
    });
  const exportArchive = () =>
    run(async () => {
      if (!selectedSave || !selectedTitle) return;
      const picked = await invoke("save.export.pick", {
        titleId: selectedTitle.titleId,
        persistentId: selectedSave.persistentId,
      });
      if (!picked.selected || !picked.confirmationToken) return;
      activate(
        await invoke("save.export.start", {
          confirmationToken: picked.confirmationToken,
        }),
      );
    });
  const cancel = () => {
  ...
  return (
    <main className="role-window manager-window">
      <header>
        <div>
          <span className="eyebrow">Account save data</span>
          <h1>Save Manager</h1>
        </div>
      ...
      <div className="split-view">
        <aside className="selection-list">
          {model.titles.map((title) => (
            <button
              key={title.titleId}
              className={title.titleId === titleId ? "selected" : ""}
              disabled={!!progress}
              onClick={() => setTitleId(title.titleId)}
            >
...
function SaveEditor({
  ...
  onImport: () => void;
  onExport: () => void;
  accounts: SaveManagerModel["accounts"];
}) {
  const validTarget = isValidSavePersistentId(targetId);
  return (
    <>
      <h2>{title.name || title.titleId}</h2>
      <code>{title.titleId}</code>
      <div className="form-grid">
        <label>
          ...
          <select
            value={persistentId}
            onChange={(event) => setPersistentId(event.target.value)}
          >
            {title.saves.map((save) => (
              <option key={save.persistentId} value={save.persistentId}>
                {save.persistentId}
                {save.accountName ? ` (${save.accountName})` : ""}
              </option>
            ))}
          </select>
        </label>
        <label>
          Transfer / import target
          <input
            list="save-accounts"
            value={targetId}
          ...
          />
          <datalist id="save-accounts">
            {accounts.map((account) => (
              <option key={account.persistentId} value={account.persistentId}>
                {account.name}
              </option>
            ))}
          </datalist>
      ...
      <div className="button-row">
        <button disabled={!persistentId || busy} onClick={onExport}>
          Export…
        </button>
        <button
          disabled={!validTarget || !persistentId || busy}
          onClick={onTransfer}
        >
          Transfer
        </button>
        <button disabled={!validTarget || busy} onClick={onImport}>
          Import…
        </button>
        <button
          className="danger"
          disabled={!persistentId || busy}
          onClick={onDelete}
        >
          Delete
        </button>
      ...
      <p className="muted">
        Import and export paths are chosen by the native file dialog; this page
        never receives or submits filesystem paths.
      </p>
    </>
  );
}
