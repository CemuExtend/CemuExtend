...
export function TextureRelationsWindow() {
  ...
  const load = useCallback(
    async (offset: number, generation: string) => {
      const sequence = ++requestSequence.current;
      setLoading(true);
      setError("");
      try {
        const result = await invoke("diagnostics.getTextureRelations", {
          generation,
          offset,
          limit: pageSize,
          activeOnly,
  ...
  return (
    <main className="tool-window diagnostic-window">
      ...
      {page?.truncated && (
        <p className="warning">Snapshot was capped to protect the UI.</p>
      )}
        ...
        <table className="diagnostic-table">
          ...
          <tbody>
            {page?.rows.map((row) => (
              <tr
                key={row.id}
                className={row.kind === "view" ? "diagnostic-view" : ""}
              >
                <td>
                  {row.kind === "view"
                    ? `↳ ${translate("View")}`
                ...
                <td>
                  {row.kind === "view"
                    ? row.dimension
                    : `${row.effectiveWidth}×${row.effectiveHeight}${row.effectiveDepth > 1 ? `×${row.effectiveDepth}` : ""}`}
                </td>
                <td>
                  {row.format}
                ...
                <td>
                  {row.sliceCount
                    ? `${row.firstSlice}–${row.firstSlice + row.sliceCount - 1}`
                    : "—"}
                </td>
                <td>
                  {row.mipCount
                    ? `${row.firstMip}–${row.firstMip + row.mipCount - 1}`
                    : "—"}
                </td>
                <td>
                  {row.kind === "texture" ? `${row.ageMilliseconds} ms` : "—"}
                </td>
                <td>
                  {[
      ...
      <footer className="diagnostic-pager">
        <button
          disabled={loading || offset === 0}
          onClick={() =>
            void load(Math.max(0, offset - pageSize), page!.generation)
          }
        >
          Previous
        </button>
        ...
        <button
          disabled={loading || !page || offset + page.rows.length >= page.total}
          onClick={() => void load(offset + pageSize, page!.generation)}
        >
          Next
        </button>
      </footer>
