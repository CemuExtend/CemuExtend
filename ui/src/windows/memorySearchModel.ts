import type { MemorySearchStatus, MemoryTypedValue, MemoryValueType } from "../bridge/contracts";
...
export function parseMemoryValue(type: MemoryValueType, input: string): MemoryTypedValue | string {
  ...
  if (bounds) {
    ...
    try {
      ...
      if (value < bounds[0] || value > bounds[1]) return `Value is outside the ${type} range.`;
    } catch { return `Enter a decimal ${type} value.`; }
  } else {
...
export function progressPercent(status: MemorySearchStatus | undefined): number {
  if (!status?.bytesTotal) return 0;
  return Math.min(100, status.bytesScanned * 100 / status.bytesTotal);
}

export function pageOffset(page: number, pageSize: number, total: number): number {
