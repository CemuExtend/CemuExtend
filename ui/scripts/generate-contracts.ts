...
if (
  new Set(manifest.windowRoles).size !== manifest.windowRoles.length ||
  new Set(manifest.implementedWindowRoles).size !==
    manifest.implementedWindowRoles.length
)
  throw new Error("window roles must be unique");
...
const cppRoles = manifest.implementedWindowRoles
  .map((role) => `\t\t"${role}"`)
  .join(",\n");
await Bun.write(
  new URL("../../src/webview/generated/WindowRoles.h", import.meta.url),
...
await Bun.write(
  new URL("../../src/webview/generated/RpcMethods.h", import.meta.url),
