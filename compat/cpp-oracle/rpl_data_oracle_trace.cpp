...
namespace
{
	...
	constexpr std::uint16_t kProviderSectionCount = 10;
	...
	constexpr std::uint32_t kExpectedMainLoaderBase = 0x10007000;
	constexpr std::uint32_t kExpectedProviderDataBase = 0x10008000;
	constexpr std::uint32_t kExpectedProviderLoaderBase = 0x10009000;
	...
	constexpr std::size_t kProviderDataSection = 2;
	constexpr std::size_t kProviderExportSection = 3;
	constexpr std::size_t kProviderSymtabSection = 4;
	constexpr std::size_t kProviderStrtabSection = 5;
	constexpr std::size_t kProviderRelaSection = 6;
	constexpr std::size_t kProviderNameSection = 7;
	constexpr std::size_t kProviderCrcSection = 8;
	constexpr std::size_t kProviderFileInfoSection = 9;

	constexpr std::string_view kUsageError =
		"cpp_rpl_data_oracle_trace: invalid arguments\n";
	constexpr std::string_view kInputError =
		"cpp_rpl_data_oracle_trace: failed to read fixtures\n";
	constexpr std::string_view kContractError =
		"cpp_rpl_data_oracle_trace: fixtures do not satisfy data contract\n";
	constexpr std::string_view kHarnessError =
		"cpp_rpl_data_oracle_trace: failed to produce canonical trace\n";
	...
	void WriteSymbol(std::vector<uint8>& image, std::size_t offset,
				 std::uint32_t value, std::uint32_t size,
				 std::uint8_t info, std::uint16_t sectionIndex)
	{
		WriteBe32(image, offset, 1);
		WriteBe32(image, offset + 4, value);
		WriteBe32(image, offset + 8, size);
		image[offset + 12] = info;
					 ...
					 std::uint32_t addend)
	{
		...
		WriteBe32(image, offset + 8, addend);
	}
	...
	std::vector<uint8> ExpectedMainImage()
	{
		constexpr char sectionNameBytes[] =
			"\0.text\0.data\0.dimport_linkmod\0.symtab\0.strtab\0.rela.text\0.shstrtab\0.crcs\0.fileinfo\0";
		...
		constexpr std::size_t data = text + 16;
		...
		std::vector<uint8> image(imageBytes, 0);
		WriteElfHeader(image, kMainSectionCount, 7, 0x02000000);
		WriteSections(image, sections);
		constexpr std::array<uint8, 16> textBytes{
			0x3c, 0x80, 0x00, 0x00, 0x38, 0x84, 0x00, 0x00,
		...
		WriteSymbol(image, symbols + 16, 0xc0000008, 8, 0x11, 3);
		std::copy(symbolNames.begin(), symbolNames.end(), image.begin() + strings);
		WriteRelocation(image, rela, 0x02000002, RPL_RELOC_HA16, 4);
		WriteRelocation(image, rela + 12, 0x02000006, RPL_RELOC_LO16, 4);
		std::copy_n(reinterpret_cast<const uint8*>(sectionNameBytes), sectionNameSize,
					image.begin() + names);
		WriteFileInfo(image, fileInfo, 2, 0x7000);
	...
	{
		constexpr char sectionNameBytes[] =
			"\0.text\0.data\0.dexports\0.symtab\0.strtab\0.rela.dexports\0.shstrtab\0.crcs\0.fileinfo\0";
		constexpr std::size_t sectionNameSize = sizeof(sectionNameBytes) - 1;
		constexpr std::size_t text = kSectionTableOffset +
			kProviderSectionCount * kSectionHeaderBytes;
		constexpr std::size_t data = text + 4;
		constexpr std::size_t exports = data + 8;
		...
		const std::array<SectionSpec, kProviderSectionCount> sections{{
			{},
			{1, SHT_PROGBITS, 6, 0x02000000, text, 4, 0, 0, 4, 0},
			{7, SHT_PROGBITS, 3, 0x10000000, data, 8, 0, 0, 4, 0},
			{13, SHT_RPL_EXPORTS, 2, 0xc0000000, exports, 23, 0, 0, 4, 0},
			{23, SHT_SYMTAB, 0, 0, symbols, 32, 5, 1, 4, 16},
			{31, SHT_STRTAB, 0, 0, strings, 8, 0, 0, 1, 0},
			{39, SHT_RELA, 0, 0, rela, 12, 4, 3, 4, 12},
			{54, SHT_STRTAB, 0, 0, names, sectionNameSize, 0, 0, 1, 0},
			{64, SHT_RPL_CRCS, 0, 0, crc, kProviderSectionCount * 4, 0, 0, 4, 4},
			{70, SHT_RPL_FILEINFO, 0, 0, fileInfo, kFileInfoBytes, 0, 0, 4, 0},
		}};
		std::vector<uint8> image(imageBytes, 0);
		WriteElfHeader(image, kProviderSectionCount, 7, 0);
		WriteSections(image, sections);
		constexpr std::array<uint8, 4> textBytes{};
		constexpr std::array<uint8, 8> dataBytes{
			0xc0, 0xde, 0xc0, 0xde, 0x00, 0x00, 0x00, 0x2a};
		constexpr std::array<uint8, 8> symbolNames{0, 'a', 'n', 's', 'w', 'e', 'r', 0};
		std::copy(textBytes.begin(), textBytes.end(), image.begin() + text);
		std::copy(dataBytes.begin(), dataBytes.end(), image.begin() + data);
		...
		WriteSymbol(image, symbols + 16, 0x10000000, 8, 0x11, 2);
		std::copy(symbolNames.begin(), symbolNames.end(), image.begin() + strings);
		WriteRelocation(image, rela, 0xc0000008, RPL_RELOC_ADDR32, 0);
		std::copy_n(reinterpret_cast<const uint8*>(sectionNameBytes), sectionNameSize,
					image.begin() + names);
		WriteFileInfo(image, fileInfo, 0, 0x1000);
		WriteCrcTable(image, sections, crc);
		return image;
	}
	...
	bool ValidateRelocation(const RPLModule& module,
						...
						std::size_t recordIndex, std::size_t recordCount,
						std::uint32_t widthBytes, std::uint32_t expectedKind,
						LoadedRelocation& loaded)
	{
		const std::size_t sectionCount = module.sectionAddressTable2.size();
		if (relaIndex >= sectionCount || expectedSymtabIndex >= sectionCount ||
			expectedTargetIndex >= sectionCount || recordIndex >= recordCount ||
			widthBytes == 0 || widthBytes > 4 ||
			SectionField(image, relaIndex, 4) != SHT_RELA ||
			SectionField(image, relaIndex, 20) !=
				recordCount * sizeof(rplRelocNew_t) ||
		...
		const auto* relocationBase = static_cast<const uint8*>(
			module.sectionAddressTable2[relaIndex].ptr);
		if (!relocationBase)
			return false;
		const void* relocationPointer =
			relocationBase + recordIndex * sizeof(rplRelocNew_t);
		...
		if (kind != expectedKind || symbolIndex != 1 || targetSize < widthBytes ||
			relocationOffset < targetVirtualAddress ||
			relocationOffset - targetVirtualAddress > targetSize - widthBytes)
			return false;
		...
		if (!IsGuestRangeAccessible(targetGuestAddress, targetSize) ||
			!IsGuestRangeAccessible(site, widthBytes) ||
	...
	struct LoadedLinkState
	{
		...
		LoadedRelocation importedHa;
		LoadedRelocation importedLo;
						   ...
						   LoadedLinkState& loaded)
	{
		...
		for (std::size_t index = 0; index < kProviderSectionCount; ++index)
		{
			const bool expectedLoaded = index == kProviderTextSection ||
				index == kProviderDataSection || index == kProviderExportSection;
		...
		if (main->regionMappingBase_text.GetMPTR() != kExpectedMainTextBase ||
			...
			main->regionSize_text != kPageBytes || main->regionSize_data != 0x7000 ||
			...
			main->fileInfo.dataRegionSize != 0x7000 ||
			...
			!IsGuestRangeAccessible(kExpectedMainDataBase, 0x7000) ||
		...
		if (provider->regionMappingBase_text.GetMPTR() != kExpectedProviderTextBase ||
			provider->regionMappingBase_data != kExpectedProviderDataBase ||
			provider->regionMappingBase_loaderInfo != kExpectedProviderLoaderBase ||
			provider->regionSize_text != kPageBytes ||
			provider->regionSize_data != kPageBytes ||
			provider->regionSize_loaderInfo != kPageBytes ||
			provider->regionOrigAddr_text != 0x02000000 ||
			provider->regionOrigAddr_data != 0x10000000 ||
			provider->fileInfo.textRegionSize != kPageBytes ||
			provider->fileInfo.dataRegionSize != kPageBytes ||
			...
			provider->exportDCount != 1 || provider->exportFDataPtr != nullptr ||
			provider->exportFCount != 0 ||
			!IsGuestRangeAccessible(kExpectedProviderTextBase, kPageBytes) ||
			!IsGuestRangeAccessible(kExpectedProviderDataBase, kPageBytes) ||
		...
		if (!ValidateGuestSection(*main, mainImage, kMainTextSection,
			...
			!ValidateGuestSection(*provider, providerImage, kProviderDataSection,
							  kExpectedProviderDataBase) ||
			...
			!ValidateRelocation(*provider, providerImage, kProviderRelaSection,
							 ...
							 provider->regionMappingBase_loaderInfo, 0, 1, 4,
							 RPL_RELOC_ADDR32,
							 loaded.local) ||
			!ValidateRelocation(*main, mainImage, kMainRelaSection,
							 kMainSymtabSection, kMainTextSection,
							 main->regionMappingBase_text.GetMPTR(), 0, 2, 2,
							 RPL_RELOC_HA16, loaded.importedHa) ||
			!ValidateRelocation(*main, mainImage, kMainRelaSection,
							 kMainSymtabSection, kMainTextSection,
							 main->regionMappingBase_text.GetMPTR(), 1, 2, 2,
							 RPL_RELOC_LO16, loaded.importedLo))
			return false;
		const uint8* providerExportPointer =
			memory_getPointerFromVirtualOffset(kExpectedProviderLoaderBase);
		if (!providerExportPointer ||
			static_cast<const void*>(provider->exportDDataPtr) !=
				static_cast<const void*>(providerExportPointer + 8))
			return false;
		loaded.resolvedSymbol = provider->regionMappingBase_data;
		return true;
	}

	bool ReadGuestBe16(std::uint32_t address, std::uint16_t& value)
	{
		if (!IsGuestRangeAccessible(address, 2))
			return false;
		const uint8* pointer = memory_getPointerFromVirtualOffset(address);
		if (!pointer)
			return false;
		value = ReadBe16(std::span<const uint8>{pointer, 2}, 0);
		return true;
	}
	...
	class TestRegistryProjection
	{
	  public:
		...
		bool IsExact() const
		{
			return m_published && rplModuleCount == 2 &&
				rplModuleList[0] == m_main && rplModuleList[1] == m_provider;
		}
	...
	bool BuildMemoryProof(MemoryProof& proof)
	{
		...
		pages.reserve(12);
		// These permission bytes are only the logical guest-memory-v1 hash
		// projection (text=R|X, data=R|W, loader metadata=R).  This link-state
		// oracle does not observe or claim C++ CPU/MMU permission parity.
		if (!AddPage(pages, kExpectedMainTextBase,
					 kPermissionRead | kPermissionExecute) ||
			!AddPage(pages, kExpectedProviderTextBase,
					 kPermissionRead | kPermissionExecute))
			return false;
		for (std::uint32_t offset = 0; offset < 0x7000; offset += kPageBytes)
			if (!AddPage(pages, kExpectedMainDataBase + offset,
						 kPermissionRead | kPermissionWrite))
				return false;
		if (!AddPage(pages, kExpectedMainLoaderBase, kPermissionRead) ||
			!AddPage(pages, kExpectedProviderDataBase,
					 kPermissionRead | kPermissionWrite) ||
	...
	std::string BuildTrace(const ModuleContract& mainContract,
					   ...
					   std::uint32_t localValue, std::int32_t localAddend,
					   std::uint32_t haSite, std::uint16_t haBefore,
					   std::uint16_t haValue, std::int32_t haAddend,
					   std::uint32_t loSite, std::uint16_t loBefore,
					   std::uint16_t loValue, std::int32_t loAddend,
					   std::uint32_t resolvedSymbol,
					   std::uint32_t relocationValue,
					   std::uint32_t mainEntry, const MemoryProof& proof)
	{
		std::string output;
		output.reserve(3600);
		AppendEnvelopePrefix(output, 0, "cafe", "system");
		output += "{\"kind\":\"event\",\"name\":\"rpx-rpl-addr16-data-validated\",\"fields\":{";
		bool first = true;
		...
		AppendTypedField(output, first, "main_relocation_0_kind", "text", "addr16_ha");
		AppendTypedField(output, first, "main_relocation_1_kind", "text", "addr16_lo");
		...
		AppendTypedField(output, first, "relocation_value", "hex32",
						 Hex32(resolvedSymbol + static_cast<std::uint32_t>(localAddend)));
		AppendTypedField(output, first, "resolved_symbol", "hex32", Hex32(resolvedSymbol));
		AppendTypedField(output, first, "width_bytes", "unsigned", "4");
		output += "}}}\n";

		AppendEnvelopePrefix(output, 2, "cafe", "system");
		output += "{\"kind\":\"event\",\"name\":\"rpx-rpl-import-relocation\",\"fields\":{";
		first = true;
		AppendTypedField(output, first, "addend", "signed", Decimal(haAddend));
		AppendTypedField(output, first, "kind", "text", "addr16_ha");
		AppendTypedField(output, first, "patch_before", "hex32", Hex32(haBefore));
		AppendTypedField(output, first, "patch_site", "hex32", Hex32(haSite));
		AppendTypedField(output, first, "patch_value", "hex32", Hex32(haValue));
		AppendTypedField(output, first, "relocation_value", "hex32", Hex32(relocationValue));
		AppendTypedField(output, first, "resolved_symbol", "hex32", Hex32(resolvedSymbol));
		AppendTypedField(output, first, "width_bytes", "unsigned", "2");
		output += "}}}\n";

		AppendEnvelopePrefix(output, 3, "cafe", "system");
		output += "{\"kind\":\"event\",\"name\":\"rpx-rpl-import-relocation\",\"fields\":{";
		first = true;
		AppendTypedField(output, first, "addend", "signed", Decimal(loAddend));
		AppendTypedField(output, first, "kind", "text", "addr16_lo");
		AppendTypedField(output, first, "patch_before", "hex32", Hex32(loBefore));
		AppendTypedField(output, first, "patch_site", "hex32", Hex32(loSite));
		AppendTypedField(output, first, "patch_value", "hex32", Hex32(loValue));
		AppendTypedField(output, first, "relocation_value", "hex32", Hex32(relocationValue));
		AppendTypedField(output, first, "resolved_symbol", "hex32", Hex32(resolvedSymbol));
		AppendTypedField(output, first, "width_bytes", "unsigned", "2");
		output += "}}}\n";

		AppendEnvelopePrefix(output, 4, "memory", "memory");
		output += "{\"kind\":\"memory_hash\",\"range\":\"rpx-rpl-addr16-data-linked-map-v1\","
				  "\"guest_address\":\"";
		...
		AppendEnvelopePrefix(output, 5, "test_harness", "terminal");
		output += "{\"kind\":\"terminal\",\"reason\":\"test_completed\","
				  "\"detail_code\":\"rpx-rpl-addr16-data-link-state-v1\"}}\n";
		return output;
	}
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		std::string trace;
		{
			...
			if (session.Main()->entrypoint != kExpectedMainTextBase ||
				session.Provider()->rplHeader.entrypoint != 0 ||
			...
			const std::uint32_t haSite = loaded.importedHa.site;
			const std::uint32_t loSite = loaded.importedLo.site;
			const std::int32_t localAddend = loaded.local.addend;
			const std::int32_t haAddend = loaded.importedHa.addend;
			const std::int32_t loAddend = loaded.importedLo.addend;
			std::uint32_t localBefore{};
			std::uint16_t haBefore{};
			std::uint16_t loBefore{};
			std::uint32_t haWordBefore{};
			std::uint32_t loWordBefore{};
			if (!ReadGuestBe32(localSite, localBefore) ||
				!ReadGuestBe16(haSite, haBefore) ||
				!ReadGuestBe16(loSite, loBefore) ||
				!ReadGuestBe32(haSite - 2, haWordBefore) ||
				!ReadGuestBe32(loSite - 2, loWordBefore))
				return Fail(kContractError, 1);
			...
			if (!session.Registry().IsExact())
				return Fail(kContractError, 1);
			RPLLoader_LinkSingleModule(session.Main(), false);
			if (!session.Registry().IsExact())
				return Fail(kContractError, 1);
			RPLLoader_LinkSingleModule(session.Provider(), false);
			std::uint32_t localValue{};
			std::uint16_t haBeforeImport{};
			std::uint16_t loBeforeImport{};
			if (!session.Registry().IsExact() ||
				!ReadGuestBe32(localSite, localValue) ||
				!ReadGuestBe16(haSite, haBeforeImport) ||
				!ReadGuestBe16(loSite, loBeforeImport))
				return Fail(kContractError, 1);
			...
			// The ordinary import scan can now see the exact provider identity and
			// its locally fixed data-export descriptor; imported sites are untouched.
			if (localValue != expectedLocal || haBeforeImport != haBefore ||
				loBeforeImport != loBefore || session.Provider()->exportDCount != 1 ||
				static_cast<std::uint32_t>(
					session.Provider()->exportDDataPtr[0].virtualOffset) != expectedLocal)
				return Fail(kContractError, 1);
			...
			std::uint16_t haValue{};
			std::uint16_t loValue{};
			std::uint32_t haWordValue{};
			std::uint32_t loWordValue{};
			std::uint32_t providerWord0{};
			std::uint32_t providerWord1{};
			if (!session.Registry().IsExact() ||
				!ReadGuestBe16(haSite, haValue) ||
				!ReadGuestBe16(loSite, loValue) ||
				!ReadGuestBe32(haSite - 2, haWordValue) ||
				!ReadGuestBe32(loSite - 2, loWordValue) ||
				!ReadGuestBe32(kExpectedProviderDataBase, providerWord0) ||
				!ReadGuestBe32(kExpectedProviderDataBase + 4, providerWord1))
				return Fail(kContractError, 1);

			const std::uint32_t relocationValue =
				resolvedSymbol + static_cast<std::uint32_t>(haAddend);
			const std::uint16_t expectedHa = static_cast<std::uint16_t>(
				(relocationValue >> 16) + ((relocationValue >> 15) & 1U));
			const std::uint16_t expectedLo =
				static_cast<std::uint16_t>(relocationValue);
			const std::uint32_t mainEntry = session.Main()->entrypoint;
			MemoryProof proof{};
			if (!BuildMemoryProof(proof))
				return Fail(kContractError, 1);

			if (proof.pageCount != 12 || proof.byteCount != 0xc000 ||
				loaded.local.kind != RPL_RELOC_ADDR32 ||
				loaded.importedHa.kind != RPL_RELOC_HA16 ||
				loaded.importedLo.kind != RPL_RELOC_LO16 ||
				localAddend != 0 || haAddend != 4 || loAddend != 4 ||
				localBefore != 0 || haBefore != 0 || loBefore != 0 ||
				localSite != kExpectedProviderLoaderBase + 8 ||
				localValue != 0x10008000 || resolvedSymbol != 0x10008000 ||
				haSite != kExpectedMainTextBase + 2 ||
				loSite != kExpectedMainTextBase + 6 ||
				relocationValue != 0x10008004 || expectedHa != 0x1001 ||
				expectedLo != 0x8004 || haValue != expectedHa || loValue != expectedLo ||
				haWordBefore != 0x3c800000 || loWordBefore != 0x38840000 ||
				haWordValue != 0x3c801001 || loWordValue != 0x38848004 ||
				providerWord0 != 0xc0dec0de || providerWord1 != 42 ||
				mainEntry != kExpectedMainTextBase)
				return Fail(kContractError, 1);

			trace = BuildTrace(mainContract, providerContract, mainImage, providerImage,
							   localSite, localBefore, localValue, localAddend,
							   haSite, haBefore, haValue, haAddend,
							   loSite, loBefore, loValue, loAddend,
							   resolvedSymbol, relocationValue, mainEntry, proof);
			if (!session.Teardown())
				return Fail(kHarnessError, 2);
		}
